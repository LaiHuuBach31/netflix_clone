...
import { deleteRating, fetchRatings } from '../../store/ratingSlice';
...
const RatingTable: React.FC = () => {
    ...
    const [RatingToDeleteId, setRatingToDeleteId] = useState<number | null>(null);
    const dispatch = useDispatch<AppDispatch>();
    const { response, loading, error } = useSelector((state: RootState) => state.rating);
    const [currentPage, setCurrentPage] = useState(1);

    const debouncedFetchRatings = useCallback(
        debounce((page, keyword) => {
            dispatch(fetchRatings({ page, keyword }));
        }, 500),
        [dispatch]
    );

    useEffect(() => {
        debouncedFetchRatings(currentPage, searchRating);
        return () => {
            debouncedFetchRatings.cancel();
        };
    }, [currentPage, searchRating, debouncedFetchRatings]);
    ...
    // edit
    const showEditModal = (rating: Rating) => {
    ...
    const handleDeleteConfirm = () => {
        console.log(RatingToDeleteId);
        
        if (RatingToDeleteId) {
            dispatch(deleteRating(RatingToDeleteId))
                .unwrap()
                .then(() => {
                    showSuccessToast('Rating deleted successfully');
                })
            dispatch(fetchRatings({ page: currentPage, keyword: searchRating }));
        }
    ...
    return (
        ...
        >
            <div className="flex justify-between items-center mb-6 items-center">
                <h2 className="text-xl font-semibold text-gray-100">Ratings List</h2>
                            ...
                            >
                                ...
                                <td className="px-6 py-4 text-center whitespace-nowrap text-sm text-gray-300">
                                    {/* <button className="text-indigo-400 hover:text-indigo-300 mr-2"
                                        onClick={() => showEditModal(rating)}
                                    >
                                        <EditOutlined style={{ fontSize: 18 }} />
                                    </button> */}
                                    <button className="text-red-400 hover:text-red-300"
                                        onClick={() => showDeleteConfirmModal(rating.id)}
                                    >
            ...
            {/* <RatingAdd isModalOpen={isAddModalOpen} onClose={handleAddModalClose} />
            {
                selectedRating !== null && (
                    <RatingEdit
