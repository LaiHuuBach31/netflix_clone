...
import { CreatePayload, Subscription } from '../../services/subscriptionService';
import VirtualList from 'rc-virtual-list';
import { showErrorToast, showSuccessToast } from '../../../../utils/toast';
import { fetchSubscriptions, updateSubscription } from '../../store/subscriptionSlice';
...
const SubscriptionEdit: React.FC<SubscriptionEditProps> = ({ isModalOpen, subscription, onClose }) => {
  ...
  useEffect(() => {
    if (isModalOpen) {
      ...
      form.setFieldsValue({
        ...
        status: subscription.status ? true : false,
      });
      dispatch(fetchUsers({ page: 1, keyword: '' }));
      dispatch(fetchPlans({ page: 1, keyword: '' }));
    }
  }, [isModalOpen, subscription, dispatch, form]);
  ...
  const onFinish = async (values: FieldType) => {
    ...
    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
      form.setFields([
        { name: 'start_date', errors: ['Start date is invalid'] },
        { name: 'end_date', errors: ['End date is invalid'] },
      ]);
      return;
    }
    ...
    try {
      ...
      await dispatch(updateSubscription({ id: subscription.id, data: payload })).unwrap();
      ...
      showErrorToast(error.message || 'Failed to update subscription');
      form.resetFields();
    }
  };
